/**
 * chdreader.cpp - CHD disc reader for rcheevos hashing
 *
 * Ported from nx-redux workspace/all/minarch/chd_reader.c. The CHD is opened from the fd
 * AchievementsHash::lookup returns (via a dup'd FILE*) instead of from a path.
 */
...
// By relative path: libretro-common/include (earlier on the include path) carries an older libchdr/chd.h whose
// chd_open_file takes an RFILE*. Quoted includes inside these headers resolve next to them, so they stay vendored.
...
typedef struct {
	chd_file* chd;
	FILE* file;			// libchdr never closes a FILE it was given; closed with the handle
	uint32_t track_num;
...
static void free_handle(chd_track_handle_t* handle) {
...
void* chdreader_open_track(const char* path, uint32_t track) {
	...
	// Only a real CHD: the wrapper tries us first for every disc path.
	uint8_t magic[8] = {0};
	if (pread(fd, magic, 8, 0) != 8 || memcmp(magic, "MComprHD", 8) != 0)
		return NULL;
	...
	chd_file* chd = NULL;
	chd_error err = chd_open_file(file, CHD_OPEN_READ, NULL, &chd);
	if (err != CHDERR_NONE) {
		fclose(file);
		return NULL;
	}
...
uint32_t chdreader_first_track_sector(void* track_handle) {
	...
	// first high-density track of a GD-ROM); chdreader_read_sector expects LBAs
